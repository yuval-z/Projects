...
for pasuk in root.iter('s'):
    ...
    if (hitpaelIndex > 1):
        if (root[i][hitpaelIndex-2].text != None):
            # Goes over all the parts of the word-the m tag
            for j in range(len(root[i][hitpaelIndex-2])):
                ana = root[i][hitpaelIndex-2][j].attrib['ana']
                bibleVectors = addAna(bibleVectors, ana, i)
                # Adds the information only once per each id
                if "phraseId" in root[i][hitpaelIndex-2][j].attrib and root[i][hitpaelIndex-2][j].attrib['phraseId'] != id:
                    id = root[i][hitpaelIndex-2][j].attrib['phraseId']
    ...
    if (root[i][hitpaelIndex-1].text != None):
        ...
        for j in range(len(root[i][hitpaelIndex-1])):
            ...
            if "phraseId" in root[i][hitpaelIndex-1][j].attrib and root[i][hitpaelIndex-1][j].attrib['phraseId'] != id:
                ...
                bibleVectors = addFunction(bibleVectors, idDict[id], i)
    if (len(root[i]) > hitpaelIndex+2):
        if (root[i][hitpaelIndex+2].text != None):
            for j in range(len(root[i][hitpaelIndex+2])):
                if ('ana' in root[i][hitpaelIndex+2][j]):
                    ana = root[i][hitpaelIndex+2][j].attrib['ana']
                    bibleVectors = addAna(bibleVectors, ana, i)
                if "phraseId" in root[i][hitpaelIndex+2][j].attrib and root[i][hitpaelIndex+2][j].attrib['phraseId'] != id:
                    id = root[i][hitpaelIndex+2][j].attrib['phraseId']
...
for sentence in filtered_sentences:
    ...
    if has_hitpael:
        ...
        # Print the token two words before the HITPAEL verb
        if hitpael_index > 1:
            token_before = sentence[hitpael_index - 2]
            upos_before = token_before.get('upos')
            xpos_before = token_before.get('xpos')
            feats_before = token_before.get('feats')
        ...
        # Print the token two words after the HITPAEL verb
        if hitpael_index < len(sentence) - 2:
            token_after = sentence[hitpael_index + 2]
            upos_after = token_after.get('upos')
            xpos_after = token_after.get('xpos')
            feats_after = token_after.get('feats')

        # Check if the tokens before HITPAEL contains each possible upos, xpos, and feats
        if contains_feature(hitpael_index - 2, 'upos', upos_map):
            feature_value = sentence[hitpael_index - 2]['upos']
            vector[upos_map[feature_value]] += 1

        if contains_feature(hitpael_index - 2, 'xpos', xpos_map):
            feature_value = sentence[hitpael_index - 2]['xpos']
            vector[xpos_map[feature_value]] += 1

        if contains_feature(hitpael_index - 2, 'feats', feats_map):
            for feat in sentence[hitpael_index - 2]['feats']:
